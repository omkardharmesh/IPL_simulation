...
#this is the list holding all the original data from the pinfobowl.csv
actual=[]
#this is the list holding only the numeric data from each row ie from index 3 onwards
bowl=[]
with open('pinfobowl.csv', 'rb') as csvfile:
...
with open('pinfobowlcluster.csv', 'w') as csvfile:
	writer = csv.writer(csvfile, delimiter=',')
	for row in actual:
		writer.writerow(row)
print "done with the bowler cluster info!"
with open('bowlclusterinfo5.csv', 'w') as csvfile:
	writer = csv.writer(csvfile, delimiter=',')
	for row in cent:
		writer.writerow(row)
